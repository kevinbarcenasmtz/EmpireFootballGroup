...
export async function processPayment(formData: FormData) {
  try {
    ...
    if (existingIdempotency) {
      ...
      if (existingIdempotency.status === 'pending') {
        ...
        if (timeSinceCreation < 30000) {
          return {
            error:
              'A payment is already being processed. Please wait a moment and check your email for confirmation.',
          };
        }
    ...
    const { error: idempotencyError } = await serviceSupabase.from('payment_idempotency').upsert(
      {
        key: idempotencyKey,
