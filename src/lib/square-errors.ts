...
type ProcessableError = ErrorWithStatusCode | ErrorWithBody | NetworkError | ServerError | Error | unknown
...
export function getSquareErrorMessage(error: ProcessableError): ErrorResult {
  ...
  if (hasBody(error) && error.body?.errors && Array.isArray(error.body.errors) && error.body.errors.length > 0) {
